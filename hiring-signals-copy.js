...
const HiringSignalEngine = (() => {
    ...
    function renderCell(value, key, row) { // Added row parameter
        ...
        if (key === 'seniorLinkedin') {
            ...
            if (employees.length === 0) return '<td class="hs-cell">-</td>';

            // Store raw text in data attribute, render button
        ...
        if (key === 'impactLevel') {
            ...
            // If text is long, use button to view, otherwise badge
            if (value.length > 50) {
                // Extract a short label if possible (e.g. "Very High: ...")
                let label = 'View Impact';
            ...
            return `<td class="hs-cell">${renderBadge(value)}</td>`;
        }
        ...
        if (key === 'outreachMessage') {
            ...
            let sendBtn = '';
            if (email) {
                // We attach full text to send button too so it can work standalone if needed
